...
async function main() {
  ...
  const outputPath = path.join(screenshotsDir, `props-${Date.now()}.png`);
  ...
  try {
    ...
    // Click on a pedal in the canvas (click on first .pedal group)
    const pedal = page.locator('g.pedal').first();
    await pedal.click();
